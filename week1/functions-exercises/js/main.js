...
const tellFortune = function( numKids, partnersName, location, jobTitle ){
  // console.log('in tellFortune()', numKids);

  const output = `You will be a ${jobTitle} in ${location}, and married to ${partnersName} with ${numKids} kids.`;
...
// console.log( tellFortune( 5, 'Craigsy', 'Sydney', 'pro surfer' )  );
...
const calculateDogAge = function( puppyAgeInHumanYears, conversionRate=7 ){

  const dogYearsEquivalent = puppyAgeInHumanYears * conversionRate;

  return dogYearsEquivalent;

}; // calculateDogAge()


const humanAgeOfDog = calculateDogAge( 3 );
